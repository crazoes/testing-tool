...
static int scap_cb_test(void)
{
	...
	read_register(buf, SCAP_CB_DATA);
	...
	unsigned char buf2[SCAP_CB_DATA];
	buf2[0] = 0x4e;
	read_register(buf2, SCAP_CB_DATA);
...
static int scap_rawdata_test(void)
{
	...
	int SCAP_RW_DATA = test_data_waterproof_mode(scap_rw_reg1, scap_rw_reg2, scap_rw_reg3);
        ...
        unsigned char buf[SCAP_RW_DATA];
	buf[0] = 0x36;

	read_register(buf, SCAP_RW_DATA);
        for( int i = 0; i < SCAP_RW_DATA; i++)
        {
                printf("%x\t", buf[i]);
        }
	...
	unsigned char buf2[SCAP_RW_DATA];
	buf2[0] = 0x36;
	read_register(buf2, SCAP_RW_DATA);
...
static int panel_differ_test(void)
{
	...
	int PD_DATA = (pd_reg1 + pd_reg2) * 2;
        ...
        unsigned char pd_buf[PD_DATA], pd2_buf[PD_DATA];
        printf("\n PD %d\n\n",PD_DATA);
	pd_buf[0] = 0x36;

	read_register(pd_buf, PD_DATA);
        for( int i = 0; i < PD_DATA; i++) {
		pd2_buf[i] = pd_buf[i] / 10;
		printf("%x\t",pd2_buf[i]);
	}
