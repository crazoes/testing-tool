...
*/
	...
	unsigned char rawdata_write1[2] = {0x0a, 0x81};
	write_register(rawdata_write1);

	unsigned char rawdata_write2[2] = {0x00, 0xc0};

	for( int i = 0; i < 3; i++)
	{
		write_register(rawdata_write2);
	...
	unsigned char rawdata_write3[2] = {0x01, 0xAA};
	write_register(rawdata_write3);

//	int READRAW_DATA = (readraw1 + readraw2) * 2;
	int READRAW_DATA = 128;
