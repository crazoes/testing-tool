...
int read_config(configuration *pconfig)
{
	...
	rawdata_max_high_tx.length = total_rawdata_max_high_tx;
	...
	rawdata_min_high_tx.length = total_rawdata_min_high_tx;
	ONE_THROUGH_16(APPEND_TX_VAL, RawData_Min_High_Tx, rawdata_min_high_tx)

	config.RawData_Max_High_Tx.array = rawdata_max_high_tx.array;
	config.RawData_Max_High_Tx.length = rawdata_max_high_tx.length;

	config.RawData_Min_High_Tx.array = rawdata_min_high_tx.array;
	config.RawData_Min_High_Tx.length = rawdata_min_high_tx.length;
	return 0;
}
