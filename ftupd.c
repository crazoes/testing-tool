...
int main(int argc, char **argv)
{
	...
	if (!version_check) {
		...
		if (fw_fd < 0)
			DIE("Unable to open %s", fw_path);

		if (fstat(fw_fd, &sb) == -1)
			DIE("Unable to stat %s", fw_path);

		fw_size = sb.st_size;

		fw_map = mmap(NULL, fw_size, PROT_READ, MAP_PRIVATE, fw_fd, 0);
		if (fw_map == MAP_FAILED)
			DIE("Unable to map %s", fw_path);
	...
	if (check_ready_for_upgrade()) {
		ERROR("Device Not ready for upgrade");
		goto out;
	}

	if (validate_chip_id()) {
		ERROR("Unexpected chip id.  Upgrade failed");
		goto out;
	}
	...
	if (set_flash_length())
		WARN("Cannot set the new flash length.\n");

	sleep (3);

	if (erase_flash())
		WARN("Device didn't confirm flash erase.\n");

	sleep (2);

	if (write_firmware(fw_map, fw_size))
		ERROR("Cannot write firmware");

	if (!get_device_checksum(&new_device_checksum))
		INFO("Checksum on device: %x\n", new_device_checksum);
	else
		WARN("Cannot fetch device firmware checksum after update\n");

	if (new_device_checksum == fw_checksum) {
		INFO("Firmware update succesful\n");
		ret = 0;
	} else
		INFO("Firmware update reported failure");

out:
	if (exit_upgrade_mode())
		DIE("Device didn't exit upgrade mode");

	current_fw_version = read_register(VERSION_REGISTER);
	if (current_fw_version == -1)
		WARN("Unable to read current firwmware version\n");
	else
		INFO("Firmware version is now: %d\n", current_fw_version);
